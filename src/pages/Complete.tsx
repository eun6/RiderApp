...
function Complete() {
  ...
  const onResponse = useCallback(async response => {
    ...
    return ImageResizer.createResizedImage(
      response.path, //파일의 경로 file:///안드로이드 경로. response.data와는 다름.
      //600x600 사이즈에 맞게 해줌.
      600,
      600,
  ...
  const onTakePhoto = useCallback(() => {
    return ImagePicker.openCamera({
      ...
      includeExif: true, //이건 왠만하면 넣어줘라. 사진의 방향을 표기해줌. -> 이미지 회전해서 원상태로 할 수 있음.
      saveToPhotos: true, //라이브러리 없는데 뜯어고쳐서 만들거임.
      //cropping 하면 간단한 크롭 가능.
    })
      .then(onResponse)
      .catch(console.log);
  }, [onResponse]);
